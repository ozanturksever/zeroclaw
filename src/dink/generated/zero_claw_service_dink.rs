...
use dink_sdk::{ServiceCaller, ServiceHandler, ServiceDefinition, Result as DinkResult};
use super::types::*;
...
pub trait ZeroClawServiceServer: Send + Sync {
    ...
    async fn stream_message(&self, req: SendMessageRequest, emit: Box<dyn Fn(Vec<u8>) -> DinkResult<()> + Send + Sync>) -> DinkResult<()>;
...
impl<C: ServiceCaller> ZeroClawServiceClient<C> {
    ...
    pub async fn send_message(&self, req: &SendMessageRequest) -> DinkResult<SendMessageResponse> {
        ...
        let resp_data = self.caller.call(&self.edge_id, "ZeroClawService", "SendMessage", &req_data).await?;
        let resp: SendMessageResponse = serde_json::from_slice(&resp_data).map_err(dink_sdk::DinkError::from)?;
        Ok(resp)
    }

    pub async fn stream_message(&self, req: &SendMessageRequest, handler: Box<dyn Fn(&[u8]) -> DinkResult<()> + Send + Sync>) -> DinkResult<Box<dyn dink_sdk::Subscription>> {
        let req_data = serde_json::to_vec(req).map_err(dink_sdk::DinkError::from)?;
        self.caller.subscribe(&self.edge_id, "ZeroClawService", "StreamMessage", &req_data, handler).await
    }

    pub async fn get_status(&self, req: &GetStatusRequest) -> DinkResult<GetStatusResponse> {
        let req_data = serde_json::to_vec(req).map_err(dink_sdk::DinkError::from)?;
        let resp_data = self.caller.call(&self.edge_id, "ZeroClawService", "GetStatus", &req_data).await?;
        let resp: GetStatusResponse = serde_json::from_slice(&resp_data).map_err(dink_sdk::DinkError::from)?;
        Ok(resp)
    }

    pub async fn recall_memory(&self, req: &RecallMemoryRequest) -> DinkResult<RecallMemoryResponse> {
        let req_data = serde_json::to_vec(req).map_err(dink_sdk::DinkError::from)?;
        let resp_data = self.caller.call(&self.edge_id, "ZeroClawService", "RecallMemory", &req_data).await?;
        let resp: RecallMemoryResponse = serde_json::from_slice(&resp_data).map_err(dink_sdk::DinkError::from)?;
        Ok(resp)
    }

    pub async fn update_config(&self, req: &UpdateConfigRequest) -> DinkResult<UpdateConfigResponse> {
        let req_data = serde_json::to_vec(req).map_err(dink_sdk::DinkError::from)?;
        let resp_data = self.caller.call(&self.edge_id, "ZeroClawService", "UpdateConfig", &req_data).await?;
        let resp: UpdateConfigResponse = serde_json::from_slice(&resp_data).map_err(dink_sdk::DinkError::from)?;
        Ok(resp)
    }

    pub async fn shutdown(&self, req: &ShutdownRequest) -> DinkResult<ShutdownResponse> {
        let req_data = serde_json::to_vec(req).map_err(dink_sdk::DinkError::from)?;
        let resp_data = self.caller.call(&self.edge_id, "ZeroClawService", "Shutdown", &req_data).await?;
        let resp: ShutdownResponse = serde_json::from_slice(&resp_data).map_err(dink_sdk::DinkError::from)?;
        Ok(resp)
    }
